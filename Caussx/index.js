...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:MAMACO_BOT\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=556993553851:+55 6993553851\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('CONECTANDO...', () => {
		start('2', 'EITA TÁ QUASE LÁ...')
	})
	client.on('open', () => {
		success('2', 'AEEE CONECTADO LEK')
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo🤙 ao grupo *${mdata.subject}*\n\nSiga as regras para não tomar ban, Espero que vc goste do grupo ❤️`
				...
				teks = `Bye bye @${num.split('@')[0]} saiu pq não aguentou a pressão kkkkkkk👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD MAMACO_BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] PROPRIETÁRIO? Este é um recurso especial para o PROPRIETÁRIO ❌',
					ownerB: '[❗] PROPRIETÁRIO? Este é um recurso especial para o PROPRIETÁRIO ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de MAMACO_BOT. Registre-se para fazer amizade com o bot MAMACO_BOT por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Victoanderjao|17\n\n──「 MAMACO_BOT 」──`,
					admin: '[❗] Não membro comun vc não pode usar esse comando, tenta dnv quando vc se tornar um adm haha! ❌',
					Badmin: ' [❗] Cadê o meu adm? Só funciona se eu virar adm! ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["556993553851@s.whatsapp.net","556993426439@s.whatsapp.net","553194549739@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"556993553851@s.whatsapp.net","556993426439@s.whatsapp.net","553194549739@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["556993553851@s.whatsapp.net","556993426439@s.whatsapp.net","553194549739@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["556993553851@s.whatsapp.net","556993426439@s.whatsapp.net","553194549739@s.whatsapp.net"]//ubah nomor lo
			const premium = ["556993553851@s.whatsapp.net","556993426439@s.whatsapp.net","553194549739@s.whatsapp.net"] 
...
axios.get(`https://www.receitaws.com.br/v1/cnpj/${aris}`).then((res) => {
	conn.sendMessage(id, '[❗] CALMA AE LEK', MessageType.text)
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('Não fale essas coisas, más como vc é admin não vou banir vc😉')
	...
	if (messagesC.includes("fdp")){
			client.updatePresence(from, Presence.composing)
			reply("Não sou seu irmão")
	}
	
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("Você tem um chifre maior que de um boi, e ainda quer chamar os outros de corno hahaha")
	}
		...
		if (messagesC.includes("bot")){
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("mamacobot")){
			client.updatePresence(from, Presence.composing)
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('556993553851@s.whatsapp.net','556993426439@s.whatsapp.net','553194549739@s.whatsapp.net', options, text, {quoted: mek})
...
break
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						client.sendMessage(from,`⚠️ *ATENÇÃO* ⚠️ qualquer membro que mandar um link será expulso do grupo`, text)
					} else if (Number(args[0]) === 0) {
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS MAMACO_BOT 👑* 」\n\`\`\``
					no = 0
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo administrador @${nomor.split("@s.whatsapp.net")[0]}\nsekarang *apenas administradores* quem podem enviar mensagens`,
					contextInfo: { mentionedJid: [nomor] }
					}
...
// recursos anime, random @Mamaco domina vida
              ...
              case 'cerpen':
               ...
               case 'puisiimg':
				...
				case 'setppbot':
				client.updatePresence(from, Presence.composing) 
				if (!isQuotedImage) return reply(`Envie fotos com legendas ${prefix}setbotpp ou tags de imagem que já foram enviadas, tendeu?`)
					if (!isOwner) return reply(mess.only.ownerB)
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "Dono", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'f':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas *.f* ou marque uma imagem que já foi enviada, tendeu?`)
					}
					break
				case 'tts':
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					client.sendMessage(from, ssha, image, {caption: 'A', quoted: mek})
					break
				case 'minion':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					client.sendMessage(from, minn, image, {caption: 'A', quoted: mek})
					break
				case 'neon':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					client.sendMessage(from, nooe, image, {caption: 'A', quoted: mek})
					break
				case 'neongreen': 
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					client.sendMessage(from, gree, image, {caption: 'A', quoted: mek})
					break
				case 'neon2':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					client.sendMessage(from, duaa, image, {caption: 'A', quoted: mek})
					break
				case '3d':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
					...
					client.sendMessage(from, tigaa, image, {caption: 'A', quoted: mek})
					break
				case 'walpaperanime':
				    try {
			    	if (!isOwner) return reply(mess.only.ownerB)
						...
						client.sendMessage(from, bufferttt, image, {quoted: mek, caption: 'A'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERRO* ❌')
					}
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT MAMACO_BOT ✨* 」\n*│+ wa.me/556993553851*╰──────*「 *MAMACO_BOT* 」*────*\n\n*_SE QUER SER ADMIN DO MAMACO_BOT_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Vou ali no banheiro'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Oia isso'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Vou ali no banheiro'})
					buffer = await getBuffer(`https://fotosdemulheresnuas.net/wp-content/uploads/2018/12/Mia-Khalifa-fotos-6.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Caramba'})
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ula la'})
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Slk tiooo'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Vou ali no banheiro'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ui cresceu o bagui'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Vou ali no banheiro'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Vai pro banheiro logo'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.pinimg.com/originals/24/5f/91/245f91208f9030724dbc8d1bede2e9ff.gif`)
					client.sendMessage(from, buffer, sticker, {quoted: mek, caption: 'slk'})
					break
				case 'bot':
			     	memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/dPUVFF6.png`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*_Comandos basicos para bot:_*\n\n*pkg upgrade && pkg update*\n*pkg install git*\n*git clone (link da git)*\n*cd (repositório)*\n*bash install.sh*\n*npm start*\n\n*MAMACO_BOT NO CONTROLE*'})
					break
				case 'belle3':
					memein = await kagApi.memeindo()
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'olha p isso mano, slk '})
					break
				case 'akeno':
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Lolis, lolis e mais lolis'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/8U9GwX4.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Oia só esse 2d'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
        ...
        case 'promover':
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'Esse(a) jumento(a) escreveu um comando errado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
